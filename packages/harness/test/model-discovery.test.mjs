...
it("loads models from an OpenAI-compatible endpoint and saves them", async () => {
  ...
  const saved = JSON.parse(await readFile(path.join(home, "models.json"), "utf8"));
  assert.deepEqual(saved.gateway.models.map((model) => model.id), ["plain", "routed"]);
});
...
it("fails when the endpoint is down and nothing matching was saved", async () => {
  listing = { object: "list", data: [{ id: "remote", object: "model" }] };
  await writeConfig({ gateway: { baseUrl: `${origin}/v1`, type: "responses", apiKey: "k" } });
  await loadModelConfig(home);
  listing = null;
  await writeConfig({ gateway: { baseUrl: `${origin}/v2`, type: "responses", apiKey: "k" } });
